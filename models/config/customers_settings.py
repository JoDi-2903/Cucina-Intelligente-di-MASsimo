class CustomersSettings:
    ...
    def __init__(self, config: dict[str, int] = None):
        ...
        if config is not None:
            self.__max_customer_agents_per_step: int = config["max_customer_agents_per_step"]
            ...
            self.__max_customer_agents_per_step: int = 10
            self.__max_customers_per_agent: int = 15
            self.__time_min: int = 20
            self.__time_max: int = 100

    @property
    def max_customer_agents_per_step(self) -> int:
        return self.__max_customer_agents_per_step
